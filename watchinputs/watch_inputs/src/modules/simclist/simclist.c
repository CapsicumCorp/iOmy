...
/*
 * Changes
 * Matthew Stapleton <matthew@capsicumcorp.com>
 * Added two functions: locked_list_iterator_start and locked_list_iterator_stop to apply a mutex lock around an iterator loop to
 *   allow multiple threads to use iterators without worrying about their own locking.
 * Added code to work with Android
 * Added jni functions to link to Android
 */
...
static simclist_ifaceptrs_ver_1_t simclist_ifaceptrs_ver_1={
list_init,
list_destroy,
...
list_sort,
list_iterator_start,
locked_list_iterator_start,
list_iterator_next,
list_iterator_hasnext,
list_iterator_stop,
locked_list_iterator_stop,
list_hash,
...
static moduleiface_ver_1_t simclist_ifaces[]={
  {
    &simclist_ifaceptrs_ver_1,
...
static moduleinfo_ver_1_t simclist_moduleinfo_ver_1={
  MODULEINFO_VER_1,
  "simclist",
  NULL,
  NULL,
...
moduleinfo_ver_generic_t *simclist_getmoduleinfo() {
  return (moduleinfo_ver_generic_t *) &simclist_moduleinfo_ver_1;
}

#ifdef __ANDROID__
jlong Java_com_capsicumcorp_iomy_libraries_watchinputs_SimCList_jnigetmodulesinfo( JNIEnv* env, jobject obj) {
  return (jlong) simclist_getmoduleinfo();
}
#endif
...
static int list_init(list_t *restrict l) {
    ...
    pthread_mutex_init(&l->itermutex, NULL);
    l->iter_active = 0;
    l->iter_pos = 0;
...
static void list_destroy(list_t *restrict l) {
    ...
    pthread_mutex_destroy(&l->itermutex);
}
...
static int list_prepend(list_t *restrict l, const void *data) {
...
static int locked_list_iterator_start(list_t *restrict l) {
    pthread_mutex_lock(&l->itermutex);
    return list_iterator_start(l);
}
...
static int locked_list_iterator_stop(list_t *restrict l) {
    pthread_mutex_unlock(&l->itermutex);
    return list_iterator_stop(l);
}
...
static int list_restore_file(list_t *restrict l, const char *restrict filename, size_t *restrict len) {
    int fd;
    size_t totdata;
...
static int list_comparator_string(const void *a, const void *b) { return strcmp((const char *)b, (const char *)a); }
...
static size_t list_meter_string(const void *el) { return strlen((const char *)el) + 1; }
