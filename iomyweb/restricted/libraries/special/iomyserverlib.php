...
//			$sSQL .= "   USERADDRESS_POSTALLINE3 varchar(128) not null, \n";
		...
		/*==============================================================*/
		/* Table: WatchInputs Camera Lib                                */
		/*==============================================================*/
		case 'WiCameraLib':
			$sSQL .= "create table `".$sDBName."`.`WICAMERALIB` \n";
			$sSQL .= "( \n";
			...
			$sSQL .= "   `WICAMERALIB_LASTUPDATE` bigint not null comment 'Unix Timestamp of when this row was last updated', \n";
			$sSQL .= "   primary key (WICAMERALIB_PK) \n";
			$sSQL .= "); \n";
			$sSQL .= "alter table `".$sDBName."`.`WICAMERALIB` comment 'Table for the WatchInputs Camerlib'; \n";
			break;
...
*/
	...
	switch( $sViewName ) {
		...
		case "WatchInputsPremise":
			...
			$sSQL .= "	`TIMEZONE_TZ` \n";
			$sSQL .= "FROM `".$sDBName."`.`USERS` \n";
			$sSQL .= "INNER JOIN `".$sDBName."`.`PERMHUB` ON `USERS`.`USERS_PK` = `PERMHUB`.`PERMHUB_USERS_FK` \n";
			$sSQL .= "INNER JOIN `".$sDBName."`.`HUB` ON `HUB`.`HUB_PK` = `PERMHUB`.`PERMHUB_HUB_FK` \n";
			$sSQL .= "LEFT JOIN `".$sDBName."`.`PREMISE` ON `HUB`.`HUB_PREMISE_FK` = `PREMISE`.`PREMISE_PK` \n";
			$sSQL .= "LEFT JOIN `".$sDBName."`.`PREMISEADDRESS` ON `PREMISE`.`PREMISE_PK` = `PREMISEADDRESS`.`PREMISEADDRESS_PREMISE_FK` \n";
			$sSQL .= "LEFT JOIN `".$sDBName."`.`LANGUAGE` ON `PREMISEADDRESS`.`PREMISEADDRESS_LANGUAGE_FK` = `LANGUAGE`.`LANGUAGE_PK` \n";
			$sSQL .= "LEFT JOIN `".$sDBName."`.`REGION` ON `PREMISEADDRESS`.`PREMISEADDRESS_REGION_FK` = `REGION`.`REGION_PK`  \n";
			$sSQL .= "LEFT JOIN `".$sDBName."`.`TIMEZONE` ON `PREMISEADDRESS`.`PREMISEADDRESS_TIMEZONE_FK` = `TIMEZONE`.`TIMEZONE_PK` \n";
			$sSQL .= "WHERE CURRENT_USER LIKE CONCAT(`USERS_USERNAME`, '@%') AND `USERS_STATE` = -1;\n";
			break;
...
function DB_CreateDefaultData1( $sDBName ) {
	...
	if($bError===false) {
		try {
			...
			$sSQL .= "INSERT INTO `".$sDBName."`.`RSTARIFF` (RSTARIFF_PK,RSTARIFF_RSSUBCAT_FK,RSTARIFF_NAME) VALUES (398,39,'WatchInputs Controls'); \n";
			...
			$sSQL .= "INSERT INTO `".$sDBName."`.`RSTYPE` (RSTYPE_PK,RSTYPE_RSTARIFF_FK,RSTYPE_NAME,RSTYPE_MAIN) VALUES (3906,390,'Red',0); \n";
			$sSQL .= "INSERT INTO `".$sDBName."`.`RSTYPE` (RSTYPE_PK,RSTYPE_RSTARIFF_FK,RSTYPE_NAME,RSTYPE_MAIN) VALUES (3907,390,'Green',0); \n";
			$sSQL .= "INSERT INTO `".$sDBName."`.`RSTYPE` (RSTYPE_PK,RSTYPE_RSTARIFF_FK,RSTYPE_NAME,RSTYPE_MAIN) VALUES (3908,390,'Blue',0); \n";
			...
			$sSQL .= "INSERT INTO `".$sDBName."`.`RSTYPE` (RSTYPE_PK,RSTYPE_RSTARIFF_FK,RSTYPE_NAME,RSTYPE_MAIN) VALUES (3968,396,'Disabled PTZ Controls','0'); \n";
...
function InsertTheDatabaseCoreValues( $sDBName ) {
	...
	if( $bError===false ) {
		...
		//----------------------------------------------------//
		$aCoreResult = DB_InsertCore( $sDBName, "iOmy (Vanilla)", 0, 4, 13, $iCurrentUTS );
	...
	if( $bError===false ) {
		...
		//----------------------------------------------------//
		$aCoreAddonResult = DB_InsertCoreAddon( $sDBName, $iCoreId, "iOmy Schema", 0, 4, 13, $iCurrentUTS );
