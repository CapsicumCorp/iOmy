	...
	if($bError===false) {
		try {
			...
			*/
			...
			$sSQL .= "INSERT INTO `".$sDBName."`.`REGION` (`REGION_PK`, `REGION_REGIONSUBLOC_FK`, `REGION_CURRENCIES_FK`, `REGION_NAME`, `REGION_NAME2`, `REGION_NAME3`, `REGION_NAME4`) VALUES (36,  53,  1,    'Australia', 'AU', 'AUS', 'ISO 3166-2:AU'); \n";
...
function InsertTheDatabaseCoreValues( $sDBName ) {
	...
	if( $bError===false ) {
		...
		//----------------------------------------------------//
		$aCoreResult = DB_InsertCore( $sDBName, "iOmy (Vanilla)", 0, 4, 7, $iCurrentUTS );
	...
	if( $bError===false ) {
		...
		//----------------------------------------------------//
		$aCoreAddonResult = DB_InsertCoreAddon( $sDBName, $iCoreId, "iOmy Schema", 0, 4, 7, $iCurrentUTS );
