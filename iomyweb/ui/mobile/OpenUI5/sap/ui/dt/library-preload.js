...
a.getFirstDescendantByCondition=function(o,c){if(!c){throw new Error("expected condition is 'undefined' or not a function");}var C=a.getAllChildOverlays(o);for(var i=0,n=C.length;i<n;i++){var b=C[i];if(c(b)){return b;}var d=this.getFirstDescendantByCondition(b,c);if(d){return d;}}return undefined;};
a.getAllChildOverlays=function(o){var c=[],C=[];if(!o){return c;}var A=o.getAggregationOverlays();for(var i=0;i<A.length;i++){C=A[i].getChildren();if(C&&C.length>0){c=c.concat(C);}}return c;};
a.getFirstChildOverlay=function(o){var c=this.getAllChildOverlays(o);if(c.length){return c[0];}return undefined;};
a.getLastChildOverlay=function(o){var c=this.getAllChildOverlays(o);if(c.length){return c[c.length-1];}return undefined;};
...
a.prototype._onItemSelected=function(e){var s=[];var i=e.getParameter("item").data("id");this._aMenuItems.some(function(I){if(i===I.id){var d=this.getDesignTime();s=d.getSelection();I.handler(s);return true;}},this);};
a.prototype._onContextMenu=function(e){e.preventDefault();document.activeElement.blur();var o=sap.ui.getCore().byId(e.currentTarget.id);if(o&&o.isSelectable()){if(!o.isSelected()){o.setSelected(true);}this.open(e,o);e.stopPropagation();}};
...
D.prototype._onElementModified=function(e){var p=e.getParameters();if(p.type==="addOrSetAggregation"||p.type==="insertAggregation"){this._onElementOverlayAddAggregation(p.value,p.target,p.name);}else if(p.type==="setParent"){setTimeout(function(){if(!this.bIsDestroyed){this._checkIfOverlayShouldBeDestroyed(p.target);}}.bind(this),0);}};
D.prototype._onElementOverlayAddAggregation=function(C,p,A){var P=O.getOverlay(p);var o=P.getAggregationOverlay(A);if(C instanceof sap.ui.core.Element){var d=O.getOverlay(C);if(!d){var i=P.getAggregationOverlay(A).isInHiddenTree();this._createElementOverlay(C,i);}else{o.addChild(d);}}};
D.prototype._checkIfOverlayShouldBeDestroyed=function(e,p){var o=O.getOverlay(e);if(o&&(!this._isElementInRootElements(e)||e.sParentAggregationName==="dependents")){o.destroy();}};
...
E.prototype.getAggregationAction=function(a,e,b){var v;var o=this.getAggregations();var c=[];for(var s in o){if(o[s].actions&&o[s].actions[a]){v=o[s].actions[a];if(typeof v==="function"){var d=[e];if(b){d=d.concat(b);}v=v.apply(null,d);}else if(typeof(v)==="string"){v={changeType:v};}if(v){v.aggregation=s;}c.push(v);}}return c;};
...
e.prototype._onElementModified=function(o){var p=o.getParameters();var s=o.getParameters().name;if(s){var f=this.getAggregationOverlay(s);var g=f&&f.isVisible();if(g){this.fireElementModified(p);}}else if(o.getParameters().type==="setParent"){this.fireElementModified(p);}this.invalidate();};
...
P.prototype.callElementOverlayRegistrationMethods=function(e){if(this.registerElementOverlay){this.registerElementOverlay(e);}this._callAggregationOverlayRegistrationMehods(e);};
